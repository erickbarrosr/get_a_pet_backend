  ...
  static async concludeAdoption(req, res) {
    try {
      ...
      if (
        ...
        return res
          .status(422)
          .json({ message: "Você não pode concluir essa adoção." });
      }
      ...
      res.status(200).json({ message: "Parabéns! Pet adotado com sucesso!" });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Erro ao concluir sua adoção." });
    }
  }
};
